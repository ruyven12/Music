...
if (controlsFixed) {
  ...
  if (!msg) {
    ...
    function setSection2(isOpen) {
      ...
      // Move separator depending on state:
      // - collapsed: separator sits under header2 (before body3)
      // - expanded: separator sits under body3 (after body3)
      if (open2) {
        msg.insertBefore(separatorUnderHeader2, body3.nextSibling);
      } else {
        msg.insertBefore(separatorUnderHeader2, body3);
      }
    }
    ...
    // Separator that sits under header2 when collapsed, and below body3 when expanded
    const separatorUnderHeader2 = createSeparator();
    separatorUnderHeader2.style.display = "block";

    // Default (collapsed): show separator under header2
    ...
    for (const [letter, bandArr] of Object.entries(letters)) {
      for (const bandObj of bandArr) {
        ...
        for (const cand of candidates) {
          const cleanPath = cleanFolderPath(cand);
...
async function bandHasAlbumForCode(info, mmddyy) {
  ...
  const folderPath = cleanFolderPath(info.band.smug_folder);
  ...
  (async () => {
    ...
    for (let i = 0; i < albums.length; i += ROW_SIZE) {
      ...
      rowAlbums.forEach((alb) => {
        ...
        const nodeKey =
          (alb && (alb.NodeID || alb.NodeId || alb.NodeKey)) || "";
        ...
        if (show) {
          ...
          meta.textContent = [datePretty, venueText]
            .filter(Boolean)
            .join(" — ");
